...
const createAccessToken = (payload) => new Promise((resolve, reject) => jsonwebtoken_1.default.sign(payload, JWT_SECRET, { expiresIn: "1h" }, (err, token) => {
    if (err)
        reject(err);
    else
        resolve(token);
...
const createRefreshToken = (payload) => new Promise((resolve, reject) => jsonwebtoken_1.default.sign(payload, JWT_REFRESH_SECRET, { expiresIn: "1 week" }, (err, token) => {
    if (err)
        reject(err);
    else
        resolve(token);
...
const verifyRefreshToken = (token) => new Promise((resolve, reject) => {
    jsonwebtoken_1.default.verify(token, JWT_REFRESH_SECRET, (err, payload) => {
        if (err)
            reject(err);
        else
            resolve(payload);
